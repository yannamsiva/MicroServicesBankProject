...
const TransactionListPage = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:9999/transaction/customer/show-all-my-transactions', {
        headers: {
  ...
  const handleFilterByDate = async () => {
    try {
      const token = localStorage.getItem('jwt');
      const response = await axios.get('http://localhost:9999/transaction/customer/filterbydate', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: {
